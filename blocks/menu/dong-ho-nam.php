...
<div class="linkpost">
<h4><a href="index.php">Trang chủ</a> > <a href="index.php?menu=dong-ho-nam">Đồng hồ Nam</a></h4>
</div>
<?php 
	$mysqli = mysqli_connect("localhost","peterdinh018","","c9");
	$sobai = mysqli_num_rows(mysqli_query($mysqli,"select * from product where gioitinh=1 order by id desc"));
	$array = Array_Pagination($sobai,"index.php?menu=dong-ho-nam&page={page}");
	$pagination = new pagination();
	$pagination->init($array);
	$men = List_Men(($pagination->get_page()-1)*9);
	while($row = mysqli_fetch_array($men))
	{
		ob_start();
?>
<div id="box">
	...
	<div class="xemchitiet_sanpham">
		<a href="gio-hang.html"><img src="upload/tool/muahang.png"/></a>
	</div>
</div>
<?php
		$s = ob_get_clean();
